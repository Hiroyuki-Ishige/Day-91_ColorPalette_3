...
def extract_color(input_image, resize, tolerance, zoom):
    # Resize photo
    output_width = resize
    ...
    plt.savefig("./uploads/" + background)
    plt.close(fig)

    # Create dataframe
    img_real = "./uploads/" + resize_name
    ...
    # Add picture at center of donut plot
    img = mpimg.imread("./uploads/" + resize_name)
    ...
    fig.set_facecolor('white')
    ax2.axis('off')
    bg = plt.imread("./uploads/" +"background.png")
    plt.imshow(bg)
    plt.tight_layout()
    fig.savefig("./uploads/after_" + input_image)
    final_file_path = "./uploads/after_"+input_image
